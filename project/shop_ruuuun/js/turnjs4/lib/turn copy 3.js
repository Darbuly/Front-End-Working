...
(function ($) {
  ...
  var has3d,
    ...
    version = '4.1.0.1.3',//奔跑修改版本从 4.1.0 => 4.1.0.1.0 开始
    ...
    flipMethods = {
      ...
      _cornerActivated: function (p) {
        ...
        if (!point.corner || $.inArray(point.corner, allowedCorners) == -1) {
          this.tlog('point.corner || $.inArray(point.corner, allowedCorners) == -1')
          return false;
        } else {
          return point;
        }
      ...
      turnPage: function (corner) {
        ...
        //support the transvers turn
        if (data.transvers_last_point) {
          corner = data.transvers_last_point
        } else {
          corner = {
            corner: (data.corner) ?
              data.corner.corner :
      ...
      _eventStart: function (e) {
        ...
        if (!data.corner && !data.disabled &&
          ...
          var csz = data.opts.cornerSize * 1.5;
          var c = 'l';
          ...
          if (point.y < csz) { }
            ...
            if (point.x < width / 2) {
              ...
              this.tlog(" 拖动翻页")
              data.drag_flag = true;
              data.drag_start_point = { x: e.pageX, y: e.pageY, time: new Date().getTime(), corner: c };
            }
            else if (point.x >= width / 2) {
              c = 'r';
              this.tlog(" 内部横向拖动区域 ")
              ...
              if (!this.flip('isTurning')) {
                data.transvers_flag = true;
                data.transvers_start_point = { x: point.x, y: point.y, time: new Date().getTime(), corner: c };
              }
      ...
      _eventMove: function (e) {
        ...
        if (!data.disabled) {
          ...
          if (data.drag_start_point) {
            ...
            var start_point = data.drag_start_point;
          }

          var transvers_flag = data.transvers_flag;
          if (transvers_flag && data.transvers_start_point) {
            var pos = data.parent.offset();
            var offsetCorner = {
              x: touch.pageX - pos.left,
              y: touch.pageY - pos.top,
            }
            // data.transvers_last_end_poing = offsetCorner;
            flipMethods._transversFolding.call(this, offsetCorner);
          }
      ...
      _eventEnd: function (e) {
        ...
        if (data.transvers_flag && data.transvers_start_point && data.transvers_last_point) {
          if (trigger('released', this, [data.transvers_last_point]) != 'prevented') {
            flipMethods.hideFoldedPage.call(this, true);
          }

          delete data.transvers_flag;
          delete data.transvers_start_point;
          delete data.transvers_start_animate;
          delete data.transvers_last_point;
        }
      ...
      _transversFolding: function (currentPoint) {

        // 趋中偏移量
        var offsetValue = 5,
          data = this.data().f,
          start_point = data.transvers_start_point,
          last_point = data.transvers_last_point,
          transvers_flag = data.transvers_flag,
          width = this.width(),
          height = this.height(),
          centerCirclePoint = point2D(width - 1, start_point.y + offsetValue),
          thet;

        if (transvers_flag) {
          thet = Math.PI / 2 - Math.atan2(centerCirclePoint.y - currentPoint.y, centerCirclePoint.x - currentPoint.x);

          var point = calcPoint(currentPoint.x, currentPoint.y, thet * 180 / Math.PI, width, height);

          /**
           * 
           * TODO:turn.js 本身不支持平滑翻页，如需支持，这部分的代码请开放注释并替换下面那部分，
           * 但是这里有个bug，就是平滑过渡与corner切换之间的冲突问题会给动画带来一种顿挫感，
           * 不好，将来需要优化这个，暂时如此。
           * 
           */

          // if (last_point && last_point.corner != point.corner) {
          //   flipMethods._showFoldedPage.call(this, point);
          // } else {
          //   flipMethods._showFoldedPage.call(this, point, true);
          // }
          data.transvers_last_point = point;

          if (!data.transvers_start_animate) {
            flipMethods._showFoldedPage.call(this, point, true);
            data.transvers_start_animate = true;
          } else {
            flipMethods._showFoldedPage.call(this, point);
          }
        }
