	...
	if (page - offset_index < vm.turnjs_arg.orgfile_pages) {
		img.load(function () {
			...
			reflectImg(old_img_elm, Cnv, 600, function (url) {
				...
				vm.ebookApp.data().backImgs[page] = {
					'back-img': url
				}
		...
		$.ajax({
			type: 'POST',
			url: "https://bizpress.ruuuun.com/wechat/getHuacePageImg",
			data: {
			...
			success: function (data) {
				if (data.code != 200) {
					...
					var Cnv = document.getElementById('myCanvas');
					AutoRotateImg(data.data, Cnv, function (src, rotated, width, height) {
						if (rotated) {
							if (!isHorizontal) isHorizontal = true;
							if (!vm.ebookApp.turn('horizontal')) {
								setHeight2 && setHeight2(width, height);
								vm.ebookApp.turn('horizontal', true);
								respondHorizon && respondHorizon();
							}
						} else {
							if (HEIGHT == 0) {
								setHeight2 && setHeight2(width, height);
...
function AutoRotateImg(img_src, Cnv, callback) {
	...
	old_img_elm.onload = function () {
		...
		if (old_img_elm.height < old_img_elm.width || (isHorizontal && (img_src == '/images/ebook/unauthorize.jpg'))) {
			//旋转纠正宽高
			...
			callback(Pic, true, Cnv.width, Cnv.height);
		} else {
			...
			Cntx.drawImage(old_img_elm, 0, 0, old_img_elm.width, old_img_elm.height)
			var Pic = Cnv.toDataURL(FILE_TYPE);
			callback(Pic, false, Cnv.width, Cnv.height);
		}
