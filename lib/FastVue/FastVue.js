...
class FastVue {
     ...
     * 注册组件
     */
    async regComponent(componentId, initdata = {}, initMethods = {}) {
        if (!componentId) {
            this.throwError('你使用了注册组件功能，但该组件没有指定参数！')
        }

        //根据id初始化组件
        let res = await this.makeComponent(componentId, 1, initdata, initMethods)
    ...
    async wrapComponent(container, wrapComponentId, initdata = {}, initMethods = {}) {
        ...
        //初始化绑定对象
        let _VM = await this.makeComponent(wrapComponentId, container, initdata, initMethods)
    ...
    async pushComponent(container, componentId, initdata = {}, initMethods = {}) {
        ...
        let _VM = await this.makeComponent(componentId, 0, initdata, initMethods)
