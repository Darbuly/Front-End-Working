class FastVue {
    ...
    async pushComponent(container, componentId, initdata = {}) {
        console.log('放置一个组件');
        ...
        console.log('组件脚本数据信息为', _script);
        let _VM = new Vue({
            el: `#${componentId}`,
            template: _tpl.template,
            data: Object.keys(initdata) != 0 ? initdata : _script.data,
            created: _script.created,
            methods: _script.methods
        });
