	...
	return {
		...
		initMvc: function (sch_id) {
			try {
				...
				this.mvc_Cnf = this.mvc_ConfigSchemeMgr.GetConfigScheme(CpqMvc.objBroker, sch_id);
		...
		refreshMvcState: function () {
			...
			CurrentCnfNodeList.map(cItem => {
				...
				if (cItem.ChildSelectionMode == '1') {
					// 单选：只需查看默认选中的情况就行了
					const cnfNodeState = cItem.FindFirstSelectedChildNodeState();

					if (cnfNodeState && cnfNodeState.IsSelected) {
						console.log(`${cItem.Id} -> ${cnfNodeState.Id}`);
						...
						nodeVue.parentDataVue[nodeVue.nodeData.name] = String(cnfNodeState.Id);
					} else {
					...
					if (!cItem.XAllowDuplicate) {
						...
						const cnfNodeStateList = cItem.FindStatesForChildNode(nodeId);

						const cnfNodeIdSelectedIdList = [];
						cnfNodeStateList.map(cnfNodeState => {
							if (cnfNodeState.IsSelected) {
								cnfNodeIdSelectedIdList.push(String(cnfNodeState.Id));
							}
						})

						console.log(`${cItem.Id} -> ${JSON.stringify(cnfNodeIdSelectedIdList)}`);
						...
						nodeVue = mapObj.nodeVue;
						nodeVue.parentDataVue[nodeVue.nodeData.name] = cnfNodeIdSelectedIdList;
		...
		handleNodeChange(nodeData, parentDataVue) {
			...
			const currentNodesList = this.mvc_Cnf.RefreshAllNodes();
			console.log('handleNodeChange currentNodesList', currentNodesList);
			if (currentNodesList.length == 0) {
				console.log('无配置脚本');
				return;
			}
			const cnfNode = this.mvc_Cnf.GetNode(nodeData.id);
			if (!cnfNode) {
				throw new Error(`handleNodeChange getNode ${nodeData.id} fail！`)
			}
			// 获取所有节点的State
			const cnfNodeStateList = cnfNode.FindStatesForChildNode(nodeData.id);
			const formValue = parentDataVue[nodeData.name];

			// 单选
			if (!nodeData.multiple) {
				const selectedCnfNode = null;

				// 遍历 state ,如果符合当前nodeData，根据当前nodeData去决定哪个State被选中
				for (let i = 0; i < cnfNodeStateList.length; i++) {
					const cItem = cnfNodeStateList[i];
					if (cItem.Id == formValue) {
						selectedCnfNode = cItem;
						break;
					}
				}

				if (!selectedCnfNode) {
					throw new Error(`handleNodeChange get selectedCnfNode fail!`);
				}
				cnfNode.SelectChildNodeState(selectedCnfNode);
			} else if (nodeData.multiple) {

				const selectedCnfNodeList = [];
				// 多选
				if (!Array.isArray(formValue)) {
					throw new Error(`handleNodeChange formValue is not array!`);
				}
				...
				formValue.map(nodeId => {
					for (let i = 0; i < cnfNodeStateList.length; i++) {
						const cItem = cnfNodeStateList[i];
						if (cItem.Id == nodeId) {
							selectedCnfNodeList.push(cItem);
						}
					}
				})

				if (selectedCnfNodeList.length > 0) {
					selectedCnfNodeList.map(sItem => {
						cnfNode.SelectChildNodeState(sItem);
					})
				}
