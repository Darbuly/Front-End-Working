...
function returnMethods() {
	return {
		...
		buildConfigNodeTree: function (data, ast_map = null) {
			...
			const configNode2EnumItem = function (configNode) {
				...
				return {
					...
					invisible: configNode.CND_IS_VISIBLE == '0',
			...
			const formatItem = function (item) {
				return {
					...
					invisible: item.CND_IS_VISIBLE == '0',
					display_order: item.CND_DISPLAY_ORDER ? Number(item.CND_DISPLAY_ORDER) : 0,
		...
		initMvc: function (sch_id) {
			try {
				this.mvc_Cnf = CpqTable.readyDataMap["sch"][sch_id];
		...
		refreshMvcState: function () {
			...
			if (!this.mvc_Cnf) {
				throw new Error(`mvc_Cnf is empty!`);
			}
		...
		handleNodeChange(nodeData, parentNodeVue, parentDataVue) {
			...
			if (!this.mvc_Cnf) {
				throw new Error(`handleNodeChange mvc_Cnf is empty!`);
			}
