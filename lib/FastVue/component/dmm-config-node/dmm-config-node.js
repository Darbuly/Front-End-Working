	...
	return {
		...
		refreshMvcState: function () {
			...
			CurrentCnfNodeList.map(cItem => {
				...
				if (cItem.ChildSelectionMode == '1') {
					// 单选：只需查看默认选中的情况就行了
					const parentNodeState = cItem.FindFirstSelectedChildNodeState();

					if (parentNodeState && parentNodeState.IsSelected) {
						console.log(`${cItem.Id} -> ${parentNodeState.ConfigNodeId}`);
						if (!this.$root.nodeMap
							|| !this.$root.nodeMap[nodeId]) {
							throw new Error('this.$root.nodeMap is empty!');
						}
						let mapObj = this.$root.nodeMap[cItem.Id];
						let nodeVue = null;
						if (mapObj.type == 'node') {
						} else if (mapObj.type == 'enumNode') {

							// 对于自己就是枚举的，则应该找跳板节点的vue
							mapObj = this.$root.nodeMap[`JMP_${cItem.Id}`];
						}
						nodeVue = mapObj.nodeVue;
						nodeVue.parentDataVue[nodeVue.nodeData.name] = String(parentNodeState.ConfigNodeId);
					} else {
						console.log('单选无state')
					}

				} else if (cItem.ChildSelectionMode == '2') {

					// 多选：根据mvc选中状态，刷新表单值
					if (!cItem.XAllowDuplicate) {
						// 1. XAllowDuplicate = false
						const parentNodeStateList = cItem.GetAllChildNodeStates(nodeId);

						const parentNodeIdSelectedIdList = [];
						parentNodeStateList.map(parentNodeState => {
							if (parentNodeState.IsSelected) {
								parentNodeIdSelectedIdList.push(String(parentNodeState.ConfigNodeId));
							}
						})

						console.log(`${cItem.Id} -> ${JSON.stringify(parentNodeIdSelectedIdList)}`);
						...
						nodeVue.parentDataVue[nodeVue.nodeData.name] = parentNodeIdSelectedIdList;
					} else {
						// todo 2. XAllowDuplicate = true
		...
		// 树节点发送变动时，触发mvc的选择事件
		handleNodeChange(nodeData, parentDataVue) {
			if (this.dev) {
			...
			const selectedNodeId = formValue = parentDataVue[nodeData.name];

			// 单选
			if (!nodeData.multiple) {

				// 获取选中子节点的state
				let selectedNodeState = parentNode.FindFirstChildNodeState(selectedNodeId);

				if (!selectedNodeState) {
					selectedNodeState = parentNode.AddChildNodeState(selectedNodeId);
				}
				parentNode.SelectChildNodeState(selectedNodeState);
			} else if (nodeData.multiple) {
				// 多选

				// 获取当前节点的StateList
				const childNodeStateList = parentNode.GetAllChildNodeStates(this.formatNodeId(nodeData.id));

				// 把 childNodeStateList 数组变成 map 方便等下获取
				const childNodeStateMap = {};
				childNodeStateList.map(cNodeState => {
					childNodeStateMap[cNodeState.ConfigNodeId] = cNodeState;
				})

				// 处理旧值
				const formValueBef = nodeData.form_value;

				// 把之前的值变成 map 方便等下判断
				const formValueBefMap = {};
				formValueBef.map(nodeId => {
					formValueBefMap[nodeId] = true;
				})

				// 把 formValue 数组转成 map 方便等下判断
				const formValueMap = {};
				formValue.map(nodeId => {
					formValueMap[nodeId] = true;
				})

				if (formValueBef.length > formValue.length) {
					// 需要去除选中

					// 枚举所有旧值节点，找出变动的节点（选中/去除选中）,处理选中/去除选中
					formValueBef.map(cNodeId => {

						if (formValueMap[cNodeId]) {
							// 选中状态不变
						} else {

							// 去除选中
							let unSeletingNodeState = childNodeStateMap[cNodeId];
							if (!unSeletingNodeState) {
								// 不可能出现该情况
								throw new Error('unSeletingNodeState is null');
							}
							parentNode.UnselectChildNodeState(unSeletingNodeState);
						}
					...
					formValue.map(cNodeId => {

						if (formValueBefMap[cNodeId]) {
							// 选中状态不变
						} else {

							// 增加选中
							let seletingNodeState = childNodeStateMap[cNodeId];
							if (!seletingNodeState) {

								// 原 StateList 找不到，需要补充
								seletingNodeState = parentNode.AddChildNodeState(cNodeId);

							}
							parentNode.SelectChildNodeState(seletingNodeState);
						}
