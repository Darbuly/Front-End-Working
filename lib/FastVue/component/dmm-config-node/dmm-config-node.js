...
function returnMethods() {
	return {
			...
			CurrentCnfNodeList.map(cItem => {
				...
				if (cItem.ChildSelectionMode == '1') {
					...
					if (!cItem.XAllowDuplicate) {
						...
						const parentNodeStateList = cItem.FindStatesForChildNode(nodeId);
		...
		handleNodeChange(nodeData, parentDataVue) {
			...
			// 获取当前节点的父节点
			const parentNode = this.mvc_Cnf.GetNode(nodeData.id);
			if (!parentNode) {
			...
			if (!nodeData.multiple) {
				...
				// 获取选中子节点的state
				let selectedNodeState = parentNode.FindFirstChildNodeState(formValue);

				if (!selectedNodeState) {
					selectedNodeState = parentNode.AddChildNodeState(formValue);
				}
				...
				// 获取所有节点的State
				const childNodeStateList = parentNode.FindStatesForChildNode(nodeData.id);

				// 把 formValue 数组转成 map 方便得知选中状态
				const formValueMap = {};
				formValue.map(nodeId => {
					formValueMap[nodeId] = true;
				})

				// 枚举所有节点，找出变动的节点（选中/去除选中）,处理选中/去除选中
				childNodeStateList.map(cNodeState => {
					if (cNodeState.IsSelected) {
						if (formValueMap[cNodeState.ChildNodeId]) {
							// 选中状态不变
						} else {
							// 去除选中
							parentNode.UnselectChildNodeState(cNodeState);
						}
					} else {
						if (formValueMap[cNodeState.ChildNodeId]) {
							// 增加选中
							parentNode.SelectChildNodeState(cNodeState);
						}
					}
				})
