	...
	return {
			...
			CurrentCnfNodeList.map(cItem => {
				...
				if (cItem.ChildSelectionMode == '1') {
					...
					if (cnfNodeState && cnfNodeState.IsSelected) {
						console.log(`${cItem.Id} -> ${cnfNodeState.ConfigNodeId}`);
						...
						nodeVue.parentDataVue[nodeVue.nodeData.name] = String(cnfNodeState.ConfigNodeId);
					} else {
					...
					if (!cItem.XAllowDuplicate) {
						...
						cnfNodeStateList.map(cnfNodeState => {
							if (cnfNodeState.IsSelected) {
								cnfNodeIdSelectedIdList.push(String(cnfNodeState.ConfigNodeId));
							}
						})
		...
		handleNodeChange(nodeData, parentDataVue) {
			...
			if (!nodeData.multiple) {
				...
				// 获取选中子节点的state
				let selectedNodeState = cnfNode.FindFirstChildNodeState(formValue);

				if (!selectedNodeState) {
					selectedNodeState = cnfNode.AddChildNodeState(formValue);
				}
				cnfNode.SelectChildNodeState(selectedNodeState);
