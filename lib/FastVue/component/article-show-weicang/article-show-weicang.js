        ...
        show(type) {
            switch (type) {
                case 'success':
                    ...
                    this.vh_100 = window.innerHeight + 'px';
                    break;
                default:
                    this.status = 'success';
                    this.showStatus = true;
            ...
            setTimeout(() => {
                ...
                //图片样式修复
                var img_list = $(".article-show-weicang .body img")
                for (let i = 0; i < img_list.length; i++) {
                    const img_el = img_list[i];

                    //图片的下一个p 作为图片描述，小一号
                    if ($(img_el) && $(img_el).next()) {
                        $(img_el).next().css('font-size', '0.28rem')
                    }
                    //图片的上一个p 不需要margin-bottom
                    // if ($(img_el) && $(img_el).prev()) {
                    //     $(img_el).prev().css('margin-bottom', '0')
                    // }
