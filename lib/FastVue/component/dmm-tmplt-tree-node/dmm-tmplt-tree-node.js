...
function returnCreated() {
	return function () {
		...
		this.$root.nodeMap[this.nodeData.id] = this;
	}
}
...
function returnMethods() {
	return {
		...
		addRow: function () {
			...
			console.log(this.parentDataVue)
			console.log(this.nodeData)
			const oneRow = this.parentDataVue[this.nodeData.name][0];
			const _copyRow = JSON.parse(JSON.stringify(oneRow));

			Object.keys(_copyRow).map(keyName => {
				// 这里需要排除动态节点的数组，将来可以写个动态节点清空值的函数，目前不需要。
				if (!this.isDynamicNode(keyName)) {
					_copyRow[keyName] = null;
				}
			})
		...
		handleChanged: function (e) {
			...
			if (this.$root && this.$root.handleNodeChange
				...
				this.$root.handleNodeChange(this.nodeData);
			}
		...
		hasBody: function () {
			return (this.resolveChildren && this.resolveChildren.length > 0 && this.resolveChildren[0].length > 0)
		},
