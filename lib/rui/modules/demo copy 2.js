...
rui.define([], function (exports) {
	...
	var $ = rui.$
		...
		//字符常量
		, STR_ELEM = 'rui-demo', STR_HIDE = 'rui-hide', STR_DISABLED = 'rui-disabled', STR_NONE = 'rui-none'
		...
		, Class = function (options) {
			var that = this;
			that.index = ++demo.index;
			that.config = $.extend({}, that.config, demo.config, options);
			that.render();
		};
	...
	//重载实例
	demo.reload = function (id, options) {
		var that = thisModule.that[id];
		that.reload(options);
	...
	//核心入口
	demo.render = function (options) {
		var inst = new Class(options);
		return thisModule.call(inst);
	};

	exports(MOD_NAME, demo);
});
